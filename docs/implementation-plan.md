...
**Status:** Implemented first-pass renderer. Activation/copy into `/etc` remains deliberately manual until VM integration tests are added.
...
**Status:** Implemented first-pass nftables renderer/enforcer, iptables compatibility output, Fail2ban baseline, Postfix abuse snippets, queue checks, and CrowdSec deferral decision.
...
**Status:** Implemented first-pass local admin bootstrap, password hashing, session cookies, role storage, recovery codes, and audit logging. Full WebAuthn and browser/API coverage remain Phase 7 hardening work.
...
**Status:** Implemented first-pass restic backup artifacts, restore drill script, health checks, DNS drift check output, and ops policy rendering. Live alert delivery remains Phase 7 hardening work.

...
# Uploaded custom logo (binary) lives here; served at /branding/logo.
BRANDING_LOGO_DIR = STATE_DIR / "branding"
BRANDING_LOGO_PATH = BRANDING_LOGO_DIR / "logo.png"
# Bundled default asset (generated by tools/gen_logo.py) — no upload needed.
BUNDLED_LOGO = Path(__file__).resolve().parent / "static" / "branding" / "logo.png"

# ── Mailcow-inspired config files (Tier-2 editors) ──────────────────────
# Plain-text / JSON blobs under CONFIG_DIR. Deployed services read these;
# the admin UI only edits + triggers a reload. Missing file = empty editor.
SIEVE_PRE_PATH = CONFIG_DIR / "sieve" / "global_pre.sieve"
SIEVE_POST_PATH = CONFIG_DIR / "sieve" / "global_post.sieve"
TRANSPORTS_PATH = CONFIG_DIR / "transports.json"
TLS_POLICY_PATH = CONFIG_DIR / "tls_policy.json"
REWRITE_MAPS_PATH = CONFIG_DIR / "rewrite_maps.json"
...
def create_app() -> FastAPI:
    ...
    def _ui_global(context: Any) -> dict:
        ...
        # Default logo: /branding/logo serves the operator-uploaded file if
        # present, else the bundled binary asset. Operators may also point
        # branding.logo_url at their own hosted image instead.
        logo = b.logo_url.strip() or "/branding/logo"
        return {
    ...
    async def branding_logo(request: Request):
        # Serve the operator-uploaded logo if present, else the bundled mark.
        if BRANDING_LOGO_PATH.exists():
            return FileResponse(
                str(BRANDING_LOGO_PATH), media_type="image/png",
                filename="logo.png",
            )
        if BUNDLED_LOGO.exists():
            return FileResponse(
                str(BUNDLED_LOGO), media_type="image/png", filename="logo.png",
            )
        return Response(status_code=404)

    @app.post("/settings/branding/logo")
    async def settings_branding_logo(request: Request,
        ...
        if file is None or not file.filename:
            return RedirectResponse(
                url="/settings?error=No+file+selected", status_code=302)
        data = await file.read()
        # Hard ceiling: a logo is a small PNG. Reject anything oversized or
        # non-PNG to avoid storing arbitrary blobs in STATE_DIR.
        if len(data) > 256 * 1024:
            return RedirectResponse(
                url="/settings?error=Logo+too+large+max+256KB", status_code=302)
        if data[:8] != b"\x89PNG\r\n\x1a\n":
            return RedirectResponse(
                url="/settings?error=Only+PNG+logos+supported", status_code=302)
        try:
            BRANDING_LOGO_DIR.mkdir(parents=True, exist_ok=True)
            # Atomic write (temp + rename) like the rest of the config store.
            tmp = BRANDING_LOGO_PATH.with_suffix(".png.tmp")
            tmp.write_bytes(data)
            tmp.replace(BRANDING_LOGO_PATH)
            os.chmod(BRANDING_LOGO_PATH, 0o640)
            audit_log("branding_logo", actor_email(request),
                      "custom logo uploaded", client_ip(request))
        except Exception as exc:
            logger.exception("saving logo")
            return RedirectResponse(
                url="/settings?error=" + urllib.parse.quote("Save failed: " + str(exc)),
                status_code=302)
        return RedirectResponse(
            url="/settings?msg=Logo+uploaded", status_code=302)
    ...
    def _load_options() -> dict[str, object]:
        raw = _read_text_cfg(OPTIONS_PATH)
        if not raw:
            return {
        ...
        try:
            ...
            return {
                ...
                "quota_notify_pct": int(data.get("quota_notify_pct", 90)),
                "min_password_len": int(data.get("min_password_len", 12)),
                "rspamd_reject": float(data.get("rspamd_reject", 15.0)),
            }
    ...
    async def options_save(request: Request):
        ...
        form = await request.form()
        if not validate_csrf(request, form.get("csrf_token", "")):
            return _safe_redirect("/options", query={"error": "Invalid session token"})
        try:
            opts = {
                "fail2ban_enabled": bool(form.get("fail2ban_enabled")),
                "quota_notify_pct": int(form.get("quota_notify_pct", 90)),
                "min_password_len": int(form.get("min_password_len", 12)),
                "rspamd_reject": float(form.get("rspamd_reject", 15.0)),
            }
        except (ValueError, TypeError):
            return _safe_redirect("/options", query={"error": "Numbers required for thresholds"})
        _write_text_cfg(OPTIONS_PATH, json.dumps(opts, indent=2))
        audit_log("options_save", actor_email(request), "server options", client_ip(request))
        return _safe_redirect("/options", query={"msg": "Saved"})

    # ── Tier-2 config editors (mailcow-inspired) ───────────────────────────

    def _read_text_cfg(path: Path) -> str:
        """Read a config blob; return '' if absent. Never raises."""
        try:
            return path.read_text(encoding="utf-8") if path.exists() else ""
        except OSError:
            return ""

    def _write_text_cfg(path: Path, text: str) -> None:
        """Atomically write a config blob, creating parent dirs."""
        path.parent.mkdir(parents=True, exist_ok=True)
        atomic_write_text(path, text, mode=0o640)

    @app.get("/sieve-global", response_class=HTMLResponse)
    async def sieve_global_page(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()
        return templates.TemplateResponse(
            request, "sieve_global.html",
            {
                "request": request,
                "csrf_token": get_csrf_token(request),
                "pre": _read_text_cfg(SIEVE_PRE_PATH),
                "post": _read_text_cfg(SIEVE_POST_PATH),
                "error": request.query_params.get("error", ""),
                "msg": request.query_params.get("msg", ""),
            },
        )

    @app.post("/sieve-global")
    async def sieve_global_save(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()
        form = await request.form()
        if not validate_csrf(request, form.get("csrf_token", "")):
            return _safe_redirect("/sieve-global", query={"error": "Invalid session token"})
        _write_text_cfg(SIEVE_PRE_PATH, str(form.get("pre", "")))
        _write_text_cfg(SIEVE_POST_PATH, str(form.get("post", "")))
        subprocess.run(["dovecot", "reload"], capture_output=True, check=False)
        audit_log("sieve_global_save", actor_email(request), "global prefilter/postfilter", client_ip(request))
        return _safe_redirect("/sieve-global", query={"msg": "Saved"})

    @app.get("/transports", response_class=HTMLResponse)
    async def transports_page(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()
        raw = _read_text_cfg(TRANSPORTS_PATH)
        try:
            rows = json.loads(raw) if raw else []
        except (json.JSONDecodeError, ValueError):
            rows = []
        return templates.TemplateResponse(
            request, "transports.html",
            {
                "request": request,
                "csrf_token": get_csrf_token(request),
                "rows": rows,
    ...
    @app.post("/transports")
    async def transports_save(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()
        form = await request.form()
        if not validate_csrf(request, form.get("csrf_token", "")):
            return _safe_redirect("/transports", query={"error": "Invalid session token"})
        try:
            rows = json.loads(form.get("json", "[]"))
            if not isinstance(rows, list):
                raise ValueError("expected a JSON array")
        except (json.JSONDecodeError, ValueError) as e:
            return _safe_redirect("/transports", query={"error": f"Invalid JSON: {e}"})
        _write_text_cfg(TRANSPORTS_PATH, json.dumps(rows, indent=2))
        subprocess.run(["postfix", "reload"], capture_output=True, check=False)
        audit_log("transports_save", actor_email(request), f"{len(rows)} sender transports", client_ip(request))
        return _safe_redirect("/transports", query={"msg": "Saved"})

    @app.get("/mail-routing", response_class=HTMLResponse)
    async def mail_routing_page(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()

        def load_json(path: Path) -> list:
            raw = _read_text_cfg(path)
            try:
        ...
        return templates.TemplateResponse(
            request, "mail_routing.html",
            {
                "request": request,
                "csrf_token": get_csrf_token(request),
                "bcc_maps": load_json(REWRITE_MAPS_PATH),
                "tls_policy": load_json(TLS_POLICY_PATH),
                "error": request.query_params.get("error", ""),
                "msg": request.query_params.get("msg", ""),
            },
        )

    @app.post("/mail-routing")
    async def mail_routing_save(request: Request):
        if not require_role(request, "admin"):
            return login_redirect()
        form = await request.form()
        if not validate_csrf(request, form.get("csrf_token", "")):
            return _safe_redirect("/mail-routing", query={"error": "Invalid session token"})

        for name, path in (("bcc", REWRITE_MAPS_PATH), ("tls", TLS_POLICY_PATH)):
            try:
                rows = json.loads(form.get(name, "[]"))
                if not isinstance(rows, list):
                    raise ValueError("expected a JSON array")
            except (json.JSONDecodeError, ValueError) as e:
                return _safe_redirect("/mail-routing", query={"error": f"{name} invalid: {e}"})
            _write_text_cfg(path, json.dumps(rows, indent=2))

        subprocess.run(["postfix", "reload"], capture_output=True, check=False)
        audit_log("mail_routing_save", actor_email(request), "bcc + tls policy maps", client_ip(request))
        return _safe_redirect("/mail-routing", query={"msg": "Saved"})
    ...
    async def dns_page(request: Request):
        ...
        # Per-domain deep-link from the Domains table ("DNS" button).
        focus_domain = request.query_params.get("domain", "").strip()
        if focus_domain and expected_records:
            expected_records = [
                r for r in expected_records
                if focus_domain in (r.get("name", "") + "." + r.get("domain", ""))
                or r.get("domain", "") == focus_domain
            ]
